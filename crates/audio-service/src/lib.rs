...
mod tests {
    ...
    #[test]
    fn service_updates_recording_metadata() {
        let root = std::env::temp_dir().join(format!(
            "waystone-audio-service-update-{}",
            std::process::id()
        ));
        let project = root.join("audio-project.wayproject");
        let _ = fs::remove_dir_all(&root);
...
"#,
        ...
        let updated = service
            .update_recording(UpdateRecordingRequest {
                ...
                master: "audio/masters/revised.flac".to_string(),
                published: "audio/published/revised.opus".to_string(),
                feed: "feeds/revised.xml".to_string(),
                entry_id: "tag:example.invalid,2026:revised".to_string(),
                mime_type: "audio/ogg; codecs=opus".to_string(),
            })
        ...
        let metadata = service
            .inspect_recording(InspectRecordingRequest {
                path: metadata_path.clone(),
            })
            .expect("updated metadata should load");
        assert_eq!(metadata.recording.id, "note");
        assert_eq!(metadata.recording.title, "Revised Note");
        assert_eq!(metadata.recording.master, "audio/masters/revised.flac");
        assert_eq!(
            metadata.recording.published.as_deref(),
            Some("audio/published/revised.opus")
        );
