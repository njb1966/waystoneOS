...
The current implementation uses Rust crates with request and response structs. The daemon binaries are placeholders. No D-Bus activation, authorization layer, long-running lifecycle, or systemd integration exists yet.
...
| --- | --- | --- | --- | --- |
| Projects | `crates/project-service` | `services/projectd` | `org.waystone.Project1` | create, list, inspect, validate |
