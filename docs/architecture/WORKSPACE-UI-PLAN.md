...
It currently renders the operating frame and wires the Create pane to `project`, `record`, and `listen` CLI JSON output for project and recording metadata. The Create pane can load, edit, save, and locally preview the selected project's content index file. The Publish pane uses read-only `publish --dry-run --json` previews. The Operate pane uses read-only `host` and `identity` CLI JSON output for list, inspect, and validate.
