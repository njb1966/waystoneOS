...
`waystone-publishd` now exposes completed-history save/list/read over D-Bus for
local project records. Dry-run preview can now compare local publishable files
against a caller-supplied local remote-state manifest and classify
upload/delete/skip without contacting or mutating a remote.
The Qt Publish pane can pass that local manifest path into its publish preview,
validation, planned-history, saved-preview, and target-overview flows and
...
parser used for dry-run comparison. It can also export the configured
removable destination root's current file path set as a local state manifest
for later dry-run comparison. The current transfer-readiness audit is
recorded in
...
are explicit. The publish planning crate, publish service crate, and publish
...
Removable execution now copies through destination-directory temporary files
and renames into place after copy completion. It preflights stale temporary-copy
path collisions so a stale temp file does not cause partial execution.
The Qt Publish pane now displays `publish --prepare-removable-execution`
readiness through the local CLI adapter. This is read-only: it shows
destination-root, operation paths, execution readiness, and blockers without
calling `publish --execute-removable` or mutating publish destinations.
The publish service crate and publish CLI now record copy-time removable
execution failures as structured failed or partial executor results. When a
copy-time failure occurs after execution begins, `publish --execute-removable`
writes completed-history TOML from the executor result, includes per-file error
text in JSON output, and exits nonzero unless the transfer result is
`completed`.
...
- Repo-local Debian session entry, launcher wrapper, and install-layout manifest
- Workspace preview guide and golden-path manual checklist for project-owner
  orientation
...
- Workspace selector and left activity navigation hover help that maps current
  labels to implemented panes and deferred areas
...
- Create pane provides hover help for Write, Files, Recordings, and optional
  audio/feed controls
...
- Publish pane accepts an optional local remote-state manifest path and passes it to preview, validation, planned-history, saved-preview, and target-overview commands
- Publish pane renders comparison metadata plus upload, update, delete, and skip buckets in dry-run detail
...
- Publish pane labels comparison-state and preview actions more explicitly and
  uses hover help to distinguish read-only preview/history work from actual
  publishing
...
- Publish pane shows a compact per-target overview for the selected project using read-only dry-run status, method, change count, verification count, and destination
...
`waystone-publishd` exposes completed-history result-record generation and
project-local completed-history save/list/read through `BuildCompletedHistory`,
`SaveCompletedHistory`, `ListCompletedHistory`, and `ReadCompletedHistory`.
These commands and D-Bus methods are local result-recording contracts only;
...
`ValidatePublication`. This can use caller-supplied local remote-state
manifests for comparison planning, but it does not probe remote state, unlock
credentials, transfer files, or verify remote results.
...
Result after publishd completed-history save/list/read: focused checks passed
on 2026-07-20, including publish-service/publishd Rust tests, Rust workspace
tests, clippy with warnings denied, CLI JSON contract smoke, publishd D-Bus
smoke, focused Qt project smoke, broad Qt smoke, format checks, and git diff
whitespace checks.

Result after local remote-state comparison scaffolding: checks passed on
2026-07-20, including focused publish Rust tests, Rust workspace tests, clippy
with warnings denied, CLI JSON contract smoke, publishd D-Bus smoke, focused
Qt project smoke, broad Qt smoke, format checks, and git diff whitespace
checks.

Result after Qt Publish-pane comparison ergonomics: checks passed on
2026-07-20, including Rust workspace tests, clippy with warnings denied, CLI
JSON contract smoke, publishd D-Bus smoke, focused Qt project smoke, broad Qt
smoke, format checks, and git diff whitespace checks.

Result after local remote-state export and inspection helpers: checks passed on
2026-07-20, including focused publish Rust tests, Rust workspace tests, clippy
with warnings denied, CLI JSON contract smoke, publishd D-Bus smoke, focused
Qt project smoke, broad Qt smoke, format checks, and git diff whitespace
checks.
...
Result after non-mutating transfer-intent contract: checks passed on
2026-07-20, including formatting, focused publish tests, full Rust tests,
clippy with warnings denied, CLI JSON contract smoke, publishd D-Bus smoke,
...
Result after removable file-copy execution hardening: checks passed on
2026-07-21, including Rust formatting, focused publish-service tests, full
Rust tests, CLI JSON contract smoke, clippy with warnings denied, and git diff
whitespace checks.

Result after Qt removable execution readiness display: checks passed on
2026-07-21, including Rust formatting, full Rust tests, broad Qt smoke,
focused Qt project smoke, clippy with warnings denied, and git diff whitespace
checks.

Result after removable execution partial-result history: checks passed on
2026-07-21, including Rust formatting, full Rust tests, CLI JSON contract
smoke, clippy with warnings denied, and git diff whitespace checks.

Result after removable destination-state export: checks passed on 2026-07-21,
...
Result after D-Bus removable executor implementation: checks passed on
2026-07-21, including Rust formatting, full Rust tests, private-session-bus
publishd D-Bus smoke, clippy with warnings denied, and git diff whitespace
checks.
...
Result after repo-local Debian session/install-layout contract: session layout
smoke and git diff whitespace checks passed on 2026-07-21. No files were
installed outside the repository.

Result after repo-local dev-session launch smoke: session dev smoke, session
...
Result after Workspace orientation slice: Qt build, focused Qt project smoke,
broad Qt smoke, dev-run root diagnostics, and git diff whitespace checks passed
on 2026-07-21. The slice adds a project-owner preview guide, simplifies the
manual checklist around a golden path, and adds hover help plus clearer
Publish-pane preview/comparison wording.
...
- `publish --export-remote-state` emits the selected project's local publishable path set as a plain text remote-state manifest and refuses to overwrite an existing output file.
- `publish --export-removable-state` emits a removable target destination
  root's current file path set as a plain text local state manifest and refuses
  to overwrite an existing output file.
- `publish --inspect-remote-state` validates and lists local remote-state manifests without contacting a remote.
- `publish --dry-run --remote-state PATH` compares local publishable files against a caller-supplied local state manifest and reports comparison metadata plus upload/delete/skip buckets without contacting or mutating a remote.
- `publish --validate` reports publication readiness without probing remote state, transferring files, unlocking credentials, or verifying remotes.
- `publish --transfer-intent` reports whether the current validation and
...
- `publish --execute-removable` performs confirmed local/removable file-copy
  ...
  results, copies through destination-directory temporary files before renaming
...
- `org.waystone.Publish1.ExecuteRemovable` implements the ADR-0014 request and
  response shape for confirmed local/removable execution only, delegates to
  `PublishService::execute_removable`, writes completed history from executor
  results, rejects unsupported request fields, and keeps verification as
  `not-run`.
...
- The Qt Publish pane can export a selected removable target's destination-state
  manifest into a project-local `history/previews/` helper file and load that
  path into the existing remote-state comparison field.
...
- `session/waystone.desktop`, `session/waystone-session`, and
  `session/install-layout.toml` define the repo-local Debian session and future
  install-layout contract.
- `session/waystone-session` launches the selected Workspace binary, passes
  optional repo-development settings, and fails clearly when the Workspace
  binary is missing.
...
- `docs/development/WORKSPACE-PREVIEW-GUIDE.md` documents the current
  project-owner golden path, navigation map, terms, intentionally ignored
  areas, and successful basic-pass criteria.
...
- The Create pane, workspace selectors, and left activity navigation include
  hover help for current vs optional/deferred areas.
...
- The Publish pane uses clearer comparison-state and preview/save labels while
  keeping publication execution unavailable in the UI.
...
1. Project owner reruns the visible manual preview checklist and confirms the
   Create and Publish pane layout fixes plus the orientation guide.
...
Current after the Workspace orientation slice on 2026-07-21.
