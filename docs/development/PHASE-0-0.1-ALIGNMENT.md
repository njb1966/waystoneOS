...
existing prepared feed-entry sidecar from current recording metadata. It still
does not record audio, perform real Opus codec export, update or merge existing
feed XML, expose feed-entry update in Qt, or expose feed generation over D-Bus.
...
next slice should deliberately choose the Qt surface for feed-entry update or a
