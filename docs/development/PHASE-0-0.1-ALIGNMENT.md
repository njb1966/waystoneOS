...
| Publishing Model | Strong for dry-run, validation, and local-history scope | Dry-run plans, publication readiness validation, Qt validation display, feed readiness reporting with invalid feed-entry diagnostics, selected diagnostic validation detail, diagnostic handoff back to Create, blocked states, planned history generation, saved preview records, completed history result records, D-Bus completed-history result-record generation, and Publish-pane inspection exist. Remote comparison, transfer, and verification are not implemented. |
...
| Show publication history or planned transfer state | Implemented as planned history previews, saved preview records, CLI-local completed history save/list/read, and non-mutating D-Bus completed-history result-record generation |
...
completed-history result-record generation. It also has an
